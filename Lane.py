...
py.init()
cap = cv.VideoCapture(0) # Kamera Burada
screen = py.display.set_mode((800, 600))
roi = [(0, 600), (800, 600), (680, 320), (120, 320)] #Aranacak olan alan burda
class process:
    def bubblesort(arr):
        n = len(arr)
        for i in range(n-1):
    ...
    def collectlines(mask):
        lines = cv.HoughLinesP(mask, 2, np.pi / 180, 20, np.array([]), minLineLength=50, maxLineGap=0)
        slopes = []
        new_lines = []
    ...
    def compresslines(lines):
        if len(lines) == 1:
            for line in lines:
                ...
                x = (x1+x2)/2
                y = (y1+y2)/2
                alpha = process.slope2alpha(slope)
            ...
            median_alpha = process.slope2alpha(median)
            mean_alpha = process.slope2alpha(mean)
            if median_alpha > mean_alpha - 10 and median_alpha < mean_alpha + 10:
                ...
                for line in lines:
                    ...
                    if slope != median:
                        new_lines.append(line)
                x, y, s, a = process.compresslines(new_lines)
                return x, y, s, a
    ...
    def py2cv(frame):
        ...
        lines = process.collectlines(mask)
        l_lines, r_lines = process.separatelines(lines, 400)
        draw_left = True
        draw_right = True
        if len(l_lines) > 0:
            lx, ly, ls, la = process.compresslines(l_lines)
        else:
            draw_left = False
            la = 0
        if len(r_lines) > 0:
            rx, ry, rs, ra = process.compresslines(r_lines)
        else:
            draw_right = False
            ra = 0
        if draw_left:
            frame = process.drawline(frame, lx, ly, ls, 320, 600)
        if draw_right:
            frame = process.drawline(frame, rx, ry, rs, 320, 600)
        way = la+ra
        way = process.getway(way)
        print(way)
...
while running:
    ...
    if ret == True:
        screen.fill(0)
        frame, way = process.py2cv(frame) # way çıktısı burda
