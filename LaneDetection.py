...
def categorizelines(lines):
    if len(lines) == 1:
        ...
        median_alpha = slope2alpha(median)
        mean_alpha = slope2alpha(mean)
        if median_alpha > mean_alpha - 10 and median_alpha < mean_alpha + 10:
            ...
            x, y, s, a = categorizelines(new_lines)
            return x, y, s, a
...
while True:
    ...
    if ret:
        ...
        for i, line in enumerate(lines):
            ...
            elif x1 < x/2 and x2 < x/2:
                left_lines.append(line)
        if len(left_lines) > 0:
            draw_left = True
            lx, ly, ls, la = categorizelines(left_lines)
        else:
            draw_left = False
            la = 0
        if len(right_lines) > 0:
            draw_right = True
            rx, ry, rs, ra = categorizelines(right_lines)
        else:
            draw_right = False
            ra = 0
        if draw_left:
            minx = int(lx + ((int(5*y/8) - ly) / ls))
            maxx = int(lx - ((ly - y) / ls))
            cv.line(frame, (minx, int(5*y/8)), (maxx, y), (255, 255, 0), 10)
        if draw_right:
            minx = int(rx + ((int(5*y/8) - ry) / rs))
            maxx = int(rx - ((ry - y) / rs))
            cv.line(frame, (minx, int(5*y/8)), (maxx, y), (255, 255, 0), 10)
        way = la+ra
        if way < -50:
            ...
            way = "A Little bit Left"
        elif way <= 37:
            way = "Forward"
        elif way <= 50:
            way = "A Little bit Right"
        else:
            way = "Right"
